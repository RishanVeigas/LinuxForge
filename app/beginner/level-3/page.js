...
const OBJECTIVES = [
  {
    ...
    validate: (fs, _cwd, hist) =>
      hist.some((h) =>
        /^mkdir\s+(~\/workspace|workspace|\/home\/user\/workspace)$/.test(
          h.trim(),
        ),
      ) && typeof getNode(fs, "/home/user/workspace") === "object",
    successMsg:
      "mkdir creates directories instantly. No confirmation needed — just run it.",
  },
  {
    id: "touch_file",
    ...
    successMsg:
      "touch creates empty files. It also updates timestamps on existing ones.",
  },
  {
    id: "copy_file",
    ...
    successMsg:
      "cp duplicates files. The original stays; a new copy is created at the destination.",
  },
  {
    id: "move_file",
    ...
    successMsg:
      "mv renames when source and destination are in the same directory. It also moves between directories.",
  },
  {
    id: "remove_file",
    ...
    successMsg:
      "rm is permanent. Always double-check before running it. Use rm -i for confirmation prompts.",
  },
  {
    id: "mkdir_nested",
    ...
    successMsg:
      "mkdir -p is a lifesaver. It creates the full path without errors if parents already exist.",
  },
];
...
const LESSONS_NAV = [
  {
    ...
    title: "Where Am I?",
    status: "done",
    href: "/learn/beginner/level-1",
  },
  {
    level: "02",
    title: "Moving Around",
    status: "done",
    href: "/learn/beginner/level-2",
  },
  {
    level: "03",
    title: "File & Dir Management",
    status: "active",
    href: "/learn/beginner/level-3",
  },
  {
    level: "04",
...
function buildTree(node, prefix = "", depth = 0) {
  ...
  keys.forEach((key, idx) => {
    ...
    lines.push({
      text: prefix + (isLast ? "└── " : "├── ") + key + (isDir ? "/" : ""),
      isDir,
    });
    if (isDir && depth < 2) {
      lines.push(
        ...buildTree(node[key], prefix + (isLast ? "    " : "│   "), depth + 1),
      );
    }
  });
  return lines;
  ...
  if (line.type === "ls-long") {
    return (
      <div className="font-mono text-[13px] flex gap-3">
        ...
        <span
          style={{
            color: line.isDir ? "#64b5f6" : "#ccc",
            fontWeight: line.isDir ? 700 : 400,
          }}
        >
          {line.name}
          {line.isDir ? "/" : ""}
  ...
  if (line.type === "tree") {
    return (
      <div className="font-mono text-[12px] leading-[1.6]">
        {line.lines.map((l, i) => (
          <div key={i} style={{ color: l.isDir ? "#64b5f6" : "#888" }}>
            {l.text}
          </div>
        ))}
      </div>
  ...
  if (line.type === "obj-complete") {
    return (
      <div className="flex items-start gap-2 bg-[#0a1a0a] border border-[#3ddc84]/30 rounded px-3 py-2 my-1">
        ...
        <div>
          <div className="text-[12px] text-[#3ddc84] font-bold">
            {line.label}
          </div>
          <div className="text-[11px] text-[#555] mt-0.5 leading-relaxed">
            {line.msg}
          </div>
        </div>
      </div>
  ...
  if (line.type === "warning") {
    return (
      <div className="flex items-start gap-2 bg-[#1a0e00] border border-[#f59e0b]/30 rounded px-3 py-2 my-0.5">
        ...
        <div className="text-[12px] text-[#f59e0b] leading-relaxed">
          {line.text}
        </div>
      </div>
    );
  ...
  useEffect(() => {
    setOutput([
      ...
      },
      {
        ...
        type: "text",
        color: "dim",
        text: "Type 'help' to see all commands. Use 'tree' to inspect the filesystem.",
      },
      { id: 2, type: "text", color: "dim", text: "" },
      {
        ...
        type: "text",
        color: "green",
        text: "Welcome to Level 03 — File & Directory Management",
      },
      {
        id: 4,
        type: "text",
    ...
    (raw) => {
      ...
      switch (cmd) {
        ...
        case "ls": {
          ...
          const entries = Object.keys(node).filter(
            (k) => showHidden || !k.startsWith("."),
          );
          if (entries.length === 0) {
            responseLines = [
              { type: "text", color: "dim", text: "(empty directory)" },
            ];
            break;
          }
          if (longFmt) {
            responseLines = [
              {
                type: "text",
                color: "dim",
                text: "total " + entries.length * 4,
              },
              ...entries.map((e) => {
                const isDir = typeof node[e] === "object";
                const min = String(Math.floor(Math.random() * 60)).padStart(
                  2,
                  "0",
                );
                return {
        ...
        case "echo": {
          responseLines = [
            { type: "text", color: "white", text: args.join(" ") },
          ];
          break;
        }
        case "tree": {
            ...
            responseLines = [
              {
                type: "text",
                color: "red",
                text: `tree: ${targetArg || "."}: No such file or directory`,
              },
            ];
            break;
          }
        ...
        case "mkdir": {
          if (!args[0]) {
            responseLines = [
              { type: "text", color: "red", text: "mkdir: missing operand" },
            ];
            break;
          }
                ...
                responseLines = [
                  {
                    type: "text",
                    color: "red",
                    text: `mkdir: cannot create directory '${args[0]}': File exists`,
                  },
                ];
                ok = false;
                break;
              ...
              responseLines = [
                {
                  type: "text",
                  color: "red",
                  text: `mkdir: cannot create directory '${args[0]}': No such file or directory`,
                },
              ];
            ...
            responseLines = [
              {
                type: "text",
                color: "red",
                text: `touch: cannot touch '${args[0]}': No such file or directory`,
              },
            ];
          } else {
            const cloned = deepClone(currentFs);
        ...
        case "cp": {
            ...
            responseLines = [
              {
                type: "text",
                color: "red",
                text: `cp: '${args[0]}': No such file or directory`,
              },
            ];
            break;
          }
            ...
            responseLines = [
              {
                type: "text",
                color: "red",
                text: `cp: -r not specified; omitting directory '${args[0]}'`,
              },
            ];
            break;
          }
          ...
          const { parentPath: dstParent, key: dstKey } =
            getParentAndKey(dstPath);
            ...
            responseLines = [
              {
                type: "text",
                color: "red",
                text: `cp: '${args[1]}': No such file or directory`,
              },
            ];
            break;
          }
        ...
        case "mv": {
            ...
            responseLines = [
              {
                type: "text",
                color: "red",
                text: `mv: '${args[0]}': No such file or directory`,
              },
            ];
            break;
          }
          ...
          const { parentPath: srcParent, key: srcKey } =
            getParentAndKey(srcPath);
          const { parentPath: dstParent, key: dstKey } =
            getParentAndKey(dstPath);
          const dstParentNode = getNode(currentFs, dstParent);
          if (!dstParentNode || typeof dstParentNode === "string") {
            responseLines = [
              {
                type: "text",
                color: "red",
                text: `mv: '${args[1]}': No such file or directory`,
              },
            ];
            break;
          }
        ...
        case "rm": {
          if (!args[0]) {
            responseLines = [
              { type: "text", color: "red", text: "rm: missing operand" },
            ];
            break;
          }
          const recursive = flags.some(
            ...
            responseLines = [
              {
                type: "text",
                color: "red",
                text: `rm: cannot remove '${args[0]}': No such file or directory`,
              },
            ];
            break;
          }
            ...
            responseLines = [
              {
                type: "text",
                color: "red",
                text: `rm: cannot remove '${args[0]}': Is a directory (use -r)`,
              },
            ];
            break;
          }
            ...
            responseLines = [
              {
                type: "text",
                color: "red",
                text: `rm: refusing to remove '${args[0]}': protected path`,
              },
            ];
            break;
          }
          ...
          if (interactive) {
            responseLines = [
              { type: "text", color: "yellow", text: `removed '${args[0]}'` },
            ];
          } else {
            responseLines = [];
          }
        ...
        case "rmdir": {
          if (!args[0]) {
            responseLines = [
              { type: "text", color: "red", text: "rmdir: missing operand" },
            ];
            break;
          }
          const target = resolvePath(args[0], currentCwd);
            ...
            responseLines = [
              {
                type: "text",
                color: "red",
                text: `rmdir: failed to remove '${args[0]}': No such file or directory`,
              },
            ];
          } else if (typeof node === "string") {
            responseLines = [
              {
                type: "text",
                color: "red",
                text: `rmdir: failed to remove '${args[0]}': Not a directory`,
              },
            ];
            ...
            responseLines = [
              {
                type: "text",
                color: "red",
                text: `rmdir: failed to remove '${args[0]}': Directory not empty`,
              },
            ];
          } else {
            const { parentPath, key } = getParentAndKey(target);
        ...
        case "help": {
          responseLines = [
            ...
            { type: "text", color: "dim", text: "" },
            {
              type: "text",
              color: "cyan",
              text: "  — File & Directory Management —",
            },
            {
              type: "text",
              color: "white",
              text: "  mkdir <dir>       create directory",
            },
            {
              type: "text",
              color: "white",
              text: "  mkdir -p <path>   create nested directories",
            },
            {
              type: "text",
              color: "white",
            ...
            },
            {
              type: "text",
              color: "white",
              text: "  cp <src> <dst>    copy file",
            },
            {
              type: "text",
              color: "white",
              text: "  cp -r <src> <dst> copy directory",
            },
            {
              type: "text",
              color: "white",
              text: "  mv <src> <dst>    move / rename",
            },
            {
              type: "text",
              color: "white",
            ...
            },
            {
              type: "text",
              color: "white",
              text: "  rm -r <dir>       delete directory",
            },
            {
              type: "text",
              color: "white",
              text: "  rmdir <dir>       remove empty directory",
            },
            { type: "text", color: "dim", text: "" },
            { type: "text", color: "cyan", text: "  — Navigation —" },
            {
              type: "text",
              color: "white",
              text: "  cd / ls / pwd / tree / cat / echo / clear",
            },
          ];
          break;
        }
                ...
                addLines([
                  {
                    type: "text",
                    color: "green",
                    text: "✓ All objectives complete! Level 03 passed.",
                  },
                ]);
                setShowToast(true);
              }, 400);
    ...
    >
      ...
      <header className="h-11 flex-shrink-0 bg-[#0d120d] border-b border-[#1a2e1a] flex items-center justify-between px-4 z-20">
        ...
        <div className="flex items-center gap-3">
          ...
          <a
            href="/"
            className="text-xs text-[#444] hover:text-[#3ddc84] transition-colors"
          >
            exit
          </a>
        </div>
        ...
        >
          ...
          <div className="flex-1 overflow-y-auto px-5 py-4 space-y-5 scrollbar-thin">
                ...
                {LESSON.description.map((seg, i) => {
                  if (seg.code)
                    return (
                      <code
                        key={i}
                        className="text-[#3ddc84] bg-[#0a1a0a] px-1 rounded text-[11px]"
                      >
                        {seg.text}
                      </code>
                    );
                  if (seg.highlight)
                    return (
                      <span key={i} className="text-[#3ddc84]">
                        {seg.text}
                      </span>
                    );
                  ...
                  return (
                    <div
                      ...
                      className={`flex items-start gap-2.5 px-2.5 py-2 rounded text-[11px] transition-colors ${
                          ...
                          ? "bg-[#0a1a0a] border border-[#1a3a1a]"
                          : active
                            ? "bg-[#3ddc84]/5 border border-[#3ddc84]/20"
                    ...
                    >
                      ...
                      <div>
                        ...
                        {active && (
                          <div className="text-[#555] mt-0.5 leading-relaxed">
                            {obj.desc}
                          </div>
                        )}
                      </div>
              ...
              <div className="space-y-1.5">
                {LESSON.commands.map((c) => (
                  <div key={c.name} className="flex gap-3 items-baseline">
                    <code className="text-[11px] text-[#3ddc84] font-bold min-w-[80px] flex-shrink-0">
                      {c.name}
                    </code>
            ...
            <div className="bg-[#1a0e00] border border-[#f59e0b]/20 rounded p-3">
              <div className="text-[10px] text-[#f59e0b] font-bold tracking-[0.12em] uppercase mb-1.5">
                ⚠ Warning
              </div>
              <p className="text-[11px] text-[#664400] leading-[1.7]">
                <code className="text-[#f59e0b]">rm</code> is permanent. There
                is no trash or undo. Always double-check your path before
                running it. Use <code className="text-[#f59e0b]">rm -i</code> to
        ...
        <div className="flex-1 bg-[#000] flex flex-col overflow-hidden relative">
          ...
          {showToast && (
            <div className="absolute bottom-[72px] right-5 bg-[#0a1a0a] border border-[#3ddc84] rounded-lg px-5 py-4 w-72 shadow-[0_0_24px_#3ddc8418] z-10 animate-[slideUp_0.3s_ease]">
              <div className="text-[13px] text-[#3ddc84] font-bold mb-1.5">
                ✓ Level 03 complete!
              </div>
              <div className="text-[11px] text-[#666] leading-relaxed mb-1">
                You can now create, copy, move, rename, and delete files and
                directories. These five commands handle 90% of filesystem work.
              </div>
              <div className="text-[11px] text-[#3ddc84]/60 mb-3">
                +{LESSON.xp} XP earned
              </div>
